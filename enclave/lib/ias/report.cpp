                     ...
                     const std::array<uint8_t, CORE_ED25519_KEY_LEN> &ed25519_signing_public_key)
{
    ...
    std::copy(provision_public_key.begin(), provision_public_key.end(), public_keys.begin());
    std::copy(ed25519_signing_public_key.begin(),
              ed25519_signing_public_key.end(),
              std::next(public_keys.begin(), provision_public_key.size()));
                        ...
                        const std::array<uint8_t, CORE_ED25519_KEY_LEN> &ed25519_signing_public_key)
{
    ...
    const std::array<uint8_t, CORE_SHA_256_LEN> hash =
        get_public_keys_hash(provision_public_key, ed25519_signing_public_key);

    sgx_report_data_t report_data = {{0}};
    memcpy(report_data.d, hash.data(), CORE_SHA_256_LEN);
