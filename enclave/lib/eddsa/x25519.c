 ...
 */
static void ctmemswap(void *a, void *b, size_t len, uint8_t mask)
{
 ...
 *
 */
static void montgomery(struct mg *A, struct mg *B, const struct mg *C)
{
 ...
 *   P->z must be 1
 */
static void mg_scale(struct mg *out, const struct mg *P, const uint8_t x[X25519_KEY_LEN])
{
    struct mg T;
    int8_t foo;
    int i, j;
 ...
 */
static void do_x25519(uint8_t out[X25519_KEY_LEN],
...
static void do_x25519_base(uint8_t out[X25519_KEY_LEN], const uint8_t scalar[X25519_KEY_LEN])
{
 ...
 */
void DHell(uint8_t out[X25519_KEY_LEN],
