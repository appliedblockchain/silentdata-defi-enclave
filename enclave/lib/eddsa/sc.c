...
static const limb_t con_mu[K + 1] = {
    1586638968003385, 147551898491342, 4503509987107165, 4503599627370495, 4503599627370495, 255};

/* off = 8 * (16^64 - 1) / 15 mod m */
const sc_t con_off = {
    1530200761952544, 2593802592017535, 2401919790321849, 2401919801264264, 9382499223688};
 ...
 *
 */
static void sc_barrett(sc_t res, const lsc_t x)
{
    llimb_t carry;
    limb_t q[K + 1], r[K + 1];
    limb_t mask;
 ...
 */
void sc_reduce(sc_t dst, const sc_t e)
{
    lsc_t tmp;
    limb_t carry;
    int i;
 ...
 */
static int jsfdigit(unsigned int a, unsigned int b)
{
    int u = 2 - (a & 0x03);
    if (u == 2)
        return 0;
