...
static INLINE void fld_set0(fld_t res, limb_t x0)
{
    int i;
    res[0] = x0;
...
static INLINE void fld_add(fld_t res, const fld_t a, const fld_t b)
{
    int i;
...
static INLINE void fld_sub(fld_t res, const fld_t a, const fld_t b)
{
    int i;
    for (i = 0; i < FLD_LIMB_NUM; i++)
        res[i] = a[i] - b[i];
 ...
 */
static INLINE void fld_tinyscale(fld_t res, const fld_t a, limb_t x)
{
    int i;
    for (i = 0; i < FLD_LIMB_NUM; i++)
        res[i] = x * a[i];
 ...
 */
static INLINE void fld_scale2(fld_t res, const fld_t a)
{
    int i;
    for (i = 0; i < FLD_LIMB_NUM; i++)
        res[i] = a[i] << 1;
 ...
 */
static INLINE void fld_neg(fld_t res, const fld_t a)
{
    int i;
    for (i = 0; i < FLD_LIMB_NUM; i++)
        res[i] = -a[i];
