...
static void compress(uint64_t state[8], const uint8_t buf[SHA512_BLOCK_SIZE])
{
    uint64_t W[80], t;
...
void sha512_final(struct sha512 *ctx, uint8_t out[SHA512_HASH_LENGTH])
{
     ...
     */
    store_be64(ctx->buffer + SHA512_BLOCK_SIZE - 16, ctx->count >> 54);
    store_be64(ctx->buffer + SHA512_BLOCK_SIZE - 8, ((ctx->count << 7) | rest) << 3);
