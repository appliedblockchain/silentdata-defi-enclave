...
static void compress(uint64_t state[8], const uint8_t buf[SHA512_256_BLOCK_SIZE])
{
    uint64_t W[80], t;
...
void sha512_256_add(struct sha512_256 *ctx, const uint8_t *data, size_t len)
{
    if (ctx->fill > 0)
    {
        ...
        while (ctx->fill < SHA512_256_BLOCK_SIZE && len > 0)
        {
            ctx->buffer[ctx->fill++] = *data++;
            len--;
        }
        if (ctx->fill < SHA512_256_BLOCK_SIZE)
            return;
...
void sha512_256_final(struct sha512_256 *ctx, uint8_t out[SHA512_256_HASH_LENGTH])
{
    ...
    if (ctx->fill > SHA512_256_BLOCK_SIZE - 16)
    {
        while (ctx->fill < SHA512_256_BLOCK_SIZE)
            ctx->buffer[ctx->fill++] = 0;

        compress(ctx->state, ctx->buffer);
        ctx->fill = 0;
    }
    while (ctx->fill < SHA512_256_BLOCK_SIZE - 16)
        ctx->buffer[ctx->fill++] = 0;
     ...
     */
    store_be64(ctx->buffer + SHA512_256_BLOCK_SIZE - 16, ctx->count >> 54);
    store_be64(ctx->buffer + SHA512_256_BLOCK_SIZE - 8, ((ctx->count << 7) | rest) << 3);

    compress(ctx->state, ctx->buffer);

    for (i = 0; i < 4; i++)
        store_be64(out + 8 * i, ctx->state[i]); // 8->4
}
